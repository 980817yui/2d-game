...
func _physics_process(delta: float) -> void:
	...
	if contact_cooldown <= 0.0 and is_instance_valid(get_parent().player):
		if global_position.distance_to(get_parent().player.global_position) < 34.0:
...
func _draw() -> void:
	var tint := Color("d68a72") if hurt_time <= 0.0 else Color("fff0b1")
	...
	draw_arc(Vector2(0, -8), 30, 0.2, 2.9, 10, Color(1.0, 0.5, 0.42, 0.35), 2)
